const eropaCollectionData = [
    {
        collection: "Karpet Eropa",
        categories: [
            {
                title: "Karpet Eropa",
                description: "Elegant European-style rugs that bring a touch of sophistication to any room. Featuring classic patterns and luxurious materials.",
                specifications: "<div class='specifications'><strong>Specifications</strong><ul>" +
                "<li><strong>Material:</strong> Premium 1.440 GSM Velvet Nylon Yarn</li>" +
                "<li><strong>Thickness:</strong> 12 mm</li>" +
                "<li><strong>Border:</strong> Opulent Obrass Finish</li>" +
                "<li><strong>Backing:</strong> Soft, Anti-Slip Corner</li>" +
                "<li><strong>Warranty:</strong> 3 Years</li>",
                products: [
                    {
                        id: "lr-111",
                        title: "LR111 - BLUE HASTAG",
                        description: "A contemporary masterpiece, BLUE HASTAG weaves together intersecting lines in a vibrant blue palette that commands attention. The hashtag-inspired pattern creates a modern, graphic statement, while the rich terracotta undertones add warmth and balance to this bold, eye-catching design.",
                        image: "assets/catalogues/eropa/LR111.png",
                        sizes: [
                            {
                                size: "100x150cm",
                                price: 598000,
                                shopee: "https://shopee.co.id/Karpet-Lantai-Eropa-Aesthetic-100X150-cm-Permadani-Sultan-Tebal-Carpet-Bulu-Ruang-Tamu-Minimalis-i.964448777.27984875636?sp_atk=e6a167be-6e76-4f62-a9b2-bcf32d756a12&xptdk=e6a167be-6e76-4f62-a9b2-bcf32d756a12",
                                tokopedia: "https://www.tokopedia.com/royalrugsindonesia/karpet-lantai-eropa-aesthetic-100x150-cm-permadani-sultan-tebal-bulu-ruang-tamu-minimalis-1731266334704239873?extParam=src%3Dshop%26whid%3D15250858&aff_unique_id=&channel=others&chain_key=",
                                tiktok: "https://shop-id.tokopedia.com/pdp/sajadah-premium-besar-100x150-cm-royal-rugs-antislip-aesthetic/1729673708159862017?source=product_detail&enter_from=product_detail&enter_method=feed_list_more_from"
                            }
                        ]
                    },
                    {
                        id: "lr-222",
                        title: "LR222 - CLOUDY",
                        description: "Soft, ethereal patterns drift across the surface of CLOUDY, creating a dreamlike quality that's both calming and sophisticated. The warm terracotta base is enlivened by subtle variations in texture and tone, resulting in a rug that brings a sense of lightness and airiness to any room.",
                        image: "assets/catalogues/eropa/LR222.png",
                        sizes: [
                            {
                                size: "100x150cm",
                                price: 598000,
                                shopee: "https://shopee.co.id/Karpet-Lantai-Eropa-Aesthetic-100X150-cm-Permadani-Sultan-Tebal-Carpet-Bulu-Ruang-Tamu-Minimalis-i.964448777.27984875636?sp_atk=e6a167be-6e76-4f62-a9b2-bcf32d756a12&xptdk=e6a167be-6e76-4f62-a9b2-bcf32d756a12",
                                tokopedia: "https://www.tokopedia.com/royalrugsindonesia/karpet-lantai-eropa-aesthetic-100x150-cm-permadani-sultan-tebal-bulu-ruang-tamu-minimalis-1731266334704239873?extParam=src%3Dshop%26whid%3D15250858&aff_unique_id=&channel=others&chain_key=",
                                tiktok: "https://shop-id.tokopedia.com/pdp/sajadah-premium-besar-100x150-cm-royal-rugs-antislip-aesthetic/1729673708159862017?source=product_detail&enter_from=product_detail&enter_method=feed_list_more_from"
                            }
                        ]
                    },
                    {
                        id: "lr-333",
                        title: "LR333 - VALENCIA ART",
                        description: "Inspired by the architectural wonders of Valencia, this piece weaves together textured grays in a masterful composition that captures the essence of modern design. The subtle variations in tone create a dynamic surface that shifts beautifully with the light.",
                        image: "assets/catalogues/eropa/LR333.png",
                        sizes: [
                            {
                                size: "100x150cm",
                                price: 598000,
                                shopee: "https://shopee.co.id/Karpet-Lantai-Eropa-Aesthetic-100X150-cm-Permadani-Sultan-Tebal-Carpet-Bulu-Ruang-Tamu-Minimalis-i.964448777.27984875636?sp_atk=e6a167be-6e76-4f62-a9b2-bcf32d756a12&xptdk=e6a167be-6e76-4f62-a9b2-bcf32d756a12",
                                tokopedia: "https://www.tokopedia.com/royalrugsindonesia/karpet-lantai-eropa-aesthetic-100x150-cm-permadani-sultan-tebal-bulu-ruang-tamu-minimalis-1731266334704239873?extParam=src%3Dshop%26whid%3D15250858&aff_unique_id=&channel=others&chain_key=",
                                tiktok: "https://shop-id.tokopedia.com/pdp/sajadah-premium-besar-100x150-cm-royal-rugs-antislip-aesthetic/1729673708159862017?source=product_detail&enter_from=product_detail&enter_method=feed_list_more_from"
                            }
                        ]
                    },
                    {
                        id: "lr-444",
                        title: "LR444 - PATTERN LINE",
                        description: "Elegance flows through every thread of the PATTERN LINE, where delicate linear motifs dance across a canvas of warm beige. The subtle texture adds depth and dimension, creating a refined foundation that elevates any interior with understated luxury.",
                        image: "assets/catalogues/eropa/LR444.png",
                        sizes: [
                            {
                                size: "100x150cm",
                                price: 598000,
                                shopee: "https://shopee.co.id/Karpet-Lantai-Eropa-Aesthetic-100X150-cm-Permadani-Sultan-Tebal-Carpet-Bulu-Ruang-Tamu-Minimalis-i.964448777.27984875636?sp_atk=e6a167be-6e76-4f62-a9b2-bcf32d756a12&xptdk=e6a167be-6e76-4f62-a9b2-bcf32d756a12",
                                tokopedia: "https://www.tokopedia.com/royalrugsindonesia/karpet-lantai-eropa-aesthetic-100x150-cm-permadani-sultan-tebal-bulu-ruang-tamu-minimalis-1731266334704239873?extParam=src%3Dshop%26whid%3D15250858&aff_unique_id=&channel=others&chain_key=",
                                tiktok: "https://shop-id.tokopedia.com/pdp/sajadah-premium-besar-100x150-cm-royal-rugs-antislip-aesthetic/1729673708159862017?source=product_detail&enter_from=product_detail&enter_method=feed_list_more_from"
                            }
                        ]
                    },
                    {
                        id: "lr-555",
                        title: "LR555 - BLUE BLACK",
                        description: "A serene fusion of cool slate blues, soft greys, and sandy tones creates a layered, textured look. This design evokes the quiet beauty of an autumn landscape after rain, grounding the room with natural elegance and timeless charm.",
                        image: "assets/catalogues/eropa/LR555.png",
                        sizes: [
                            {
                                size: "100x150cm",
                                price: 598000,
                                shopee: "https://shopee.co.id/Karpet-Lantai-Eropa-Aesthetic-100X150-cm-Permadani-Sultan-Tebal-Carpet-Bulu-Ruang-Tamu-Minimalis-i.964448777.27984875636?sp_atk=e6a167be-6e76-4f62-a9b2-bcf32d756a12&xptdk=e6a167be-6e76-4f62-a9b2-bcf32d756a12",
                                tokopedia: "https://www.tokopedia.com/royalrugsindonesia/karpet-lantai-eropa-aesthetic-100x150-cm-permadani-sultan-tebal-bulu-ruang-tamu-minimalis-1731266334704239873?extParam=src%3Dshop%26whid%3D15250858&aff_unique_id=&channel=others&chain_key=",
                                tiktok: "https://shop-id.tokopedia.com/pdp/sajadah-premium-besar-100x150-cm-royal-rugs-antislip-aesthetic/1729673708159862017?source=product_detail&enter_from=product_detail&enter_method=feed_list_more_from"
                            }
                        ]
                    },
                    {
                        id: "lr-777",
                        title: "LR777 - KOREAN ARMY",
                        description: "A palette of sage olive, weathered stone grey, and antique ivory flows across a softly distressed abstract design, recalling the glow of late-harvest fields. Layered tones and textures adds dimension, grounding your home in comfort and style.",
                        image: "assets/catalogues/eropa/LR777.png",
                        sizes: [
                            {
                                size: "100x150cm",
                                price: 598000,
                                shopee: "https://shopee.co.id/Karpet-Lantai-Eropa-Aesthetic-100X150-cm-Permadani-Sultan-Tebal-Carpet-Bulu-Ruang-Tamu-Minimalis-i.964448777.27984875636?sp_atk=e6a167be-6e76-4f62-a9b2-bcf32d756a12&xptdk=e6a167be-6e76-4f62-a9b2-bcf32d756a12",
                                tokopedia: "https://www.tokopedia.com/royalrugsindonesia/karpet-lantai-eropa-aesthetic-100x150-cm-permadani-sultan-tebal-bulu-ruang-tamu-minimalis-1731266334704239873?extParam=src%3Dshop%26whid%3D15250858&aff_unique_id=&channel=others&chain_key=",
                                tiktok: "https://shop-id.tokopedia.com/pdp/sajadah-premium-besar-100x150-cm-royal-rugs-antislip-aesthetic/1729673708159862017?source=product_detail&enter_from=product_detail&enter_method=feed_list_more_from"
                            }
                        ]
                    }
                ]
            }
        ]
    }
];
